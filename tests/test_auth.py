...
def test_sign_up_creates_account_and_signs_in(client, db):
...
def test_sign_up_rejects_duplicate_username(client, alice):
    response = client.post(
        "/sign-up/",
...
def test_sign_up_rejects_short_password(client):
    response = client.post(
        "/sign-up/", data={"username": "shorty", "password": "abc", "confirm": "abc"}
    )
    assert b"at least 8 characters" in response.data


def test_sign_up_rejects_mismatched_confirmation(client):
    response = client.post(
        "/sign-up/",
...
def test_sign_in_with_bad_password_fails(client, alice):
    response = client.post("/sign-in/", data={"username": "alice", "password": "nope"})
    assert b"Invalid username or password" in response.data


def test_sign_in_succeeds(sign_in, alice):
    response = sign_in("alice")
...
def test_sign_out_requires_post(client, sign_in, alice):
    """A GET sign-out URL could be triggered by a prefetch or an <img> tag."""
    sign_in("alice")
    assert client.get("/sign-out/").status_code == 405
    assert client.post("/sign-out/", follow_redirects=True).status_code == 200
...
def test_next_parameter_allows_local_path(client, alice):
    response = client.post(
        "/sign-in/?next=/saved/", data={"username": "alice", "password": PASSWORD}
    )
...
def test_protected_pages_redirect_anonymous_users(client):
    for path in ("/posts/new/", "/saved/", "/drafts/", "/settings/", "/following/"):
        ...
        assert "/sign-in/" in response.headers["Location"], path
