...
from app.helpers.helper import valid_password, valid_email, make_password_hash
...
class SettingHandler(BaseHandler):
    ...
    def post(self):
        ...
        if setting_err == False:
            if delete1:
                comments = COMMENT.by_user_id(str(self.user.id))
            ...
            if delete2:
                ...
                for post in posts:
                    if str(post.id) in self.user.liked_posts:
                        self.user.liked_posts.remove(str(post.id))
                    self.user.put()
                    comments = COMMENT.by_post_id(str(post.id))
                    for comment in comments:
                        comment.delete()
